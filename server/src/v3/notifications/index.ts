...
export async function createNotificationNew(
    c: Context, {
        senderId,
        receiverId,
        type,
    ...
    switch (type) {
        case 'like':
            ...
            break;
        case 'comment':
            if (!metadata.parentPostId) throw new Error('Post ID not provided for comment');
            // entityId is of the parent post
            await handleComment(env, <NotificationPayload.Comment>{
                senderId,
                commentId: metadata.entityId,
                parentPostId: metadata.parentPostId
            });
            break;
        case 'subcomment':
            if (!metadata.parentCommentId) throw new Error('Comment ID not provided for subcomment');
            // entityId is of the parent comment
            await handleSubcomment(env, <NotificationPayload.Subcomment>{
                senderId,
                subcommentId: metadata.entityId,
