...
The backend is based on cloudflare workers and uses the Hono framework for building APIs. It integrates with Cloudflare D1 for database management, Resend for email services, and Google OAuth for authentication.
...
- Resend API: Email service, configured with `RESEND_API_KEY` or the legacy `EMAIL_API` secret.
...
The deploy script uses `wrangler deploy --keep-vars`, so dashboard-managed environment variables are preserved during deploys. Wrangler deploys do not delete Worker secrets. The tracked `wrangler.toml.example` includes `keep_vars = true`; keep the same setting in the environment's actual `wrangler.toml` if deploying with plain `wrangler deploy`.
